...
처음에는 캐싱으로 offset 깊이에 따른 조회속도를 개선하려고 했지만,

그렇게 되면 캐싱으로 저장해야 할 데이터의 범위가 커지고, 관리가 힘들어진다.

즉, 캐싱의 본래 목적인 요청이 많은 특정 페이지 (ex. 첫 페이지) 같이 조회 요청이 많은 페이지를 대상으로 캐싱을 진행해보자.

캐싱은 offset 이 0인 데이터에 대해서 Redis 에 캐싱 데이터를 확인하고, 있으면 캐싱 데이터를 출력하고,

없으면 DB 데이터를 출력하고, 캐싱을 진행하도록 설계했다.

즉, 최초 요청은 DB, 이후 요청은 Redis 로 설계했다.

그럼 VUS 200에 대해서 지표를 비교해보자

| VU  | avg latency | p95      | RPS  | 캐싱 여부 |
| --- | ----------- | -------- | ---- | --------- |
| 200 | 130.6ms     | 343.2ms  | 1606 | X         |
| 200 | 63.37ms     | 76.23ms  | 3256 | O         |
| 300 | 192ms       | 578.33ms | 1654 | X         |
| 300 | 91.9ms      | 101.42ms | 2898 | O         |
| 800 | 529ms       | 1590ms   | 1514 | X         |
| 800 | 245.52ms    | 288.17ms | 3251 | O         |

VUS 800 일 때를 비교했을 때, 다음과 같다
| 항목 | 캐싱 X | 캐싱 O | 개선 폭 |
| ----------- | -----: | -----------: | ------------: |
| avg latency | 529ms | **245.52ms** | **약 53.6% ↓** |
| p95 | 1590ms | **288.17ms** | **약 81.9% ↓** |
| RPS | 1514 | **3251** | **약 2.15배 ↑** |

당연한 말이지만, 수치적으로 비교해보니 Redis 의 속도감이 체감되는 지표다.

또한 동시 요청이 많으면 많을 수록 캐싱처리의 효율이 극대화 되는 것을 체감했다.

### Spring Cache vs Redis Cache ?

이전에 주식 프로젝트를 할 때, Redis 가 아닌 Spring cache 를 사용해서 주식 정보들을 저장했었는데,

Redis 를 해보면서 비교해보니

Spring cache는 @Cacheable 어노테이션으로 쉽게 구현이 가능하지만,

Redis 보다 key 제어 같은 부분이 불편해서 확장성이 떨어지는 단점이 있다는 것을 배웠다.

## 결론

Index와 Cache 를 사용해서 조회 속도 개선이 어느정도로 일어나는 지,

**Grafana + Prometeus + k6 조합**을 통해서 수치화를 해보았다.

Index로는 유의미한 지표를 내지 못했지만, Redis Cache 로는 큰 폭의 속도 향상을 볼 수 있었다.

Index 는 GPT 한테 데이터 베이스 설계를 해달라고 하면 알아서 Index 까지 붙여서 나오기 때문에,

이걸 왜 Indexing 을 적용하는 지? 에 대해서 고민이 부족했었는데,

이번 기회에 Index 의 B+Tree 자료구조 형태와, 적용하지 않았을 때와 비교해서 공부할 수 있었다.

<br>

또한 프로젝트를 개발하며 기능 구현에 바빠서 트래픽 안정성에 대한 고민도 부족했었다.

자소서에는 실제 운영을 고려한 개발을 한다고 적어왔지만, 딥하게 고민할 여유도, 노력도 부족했었다.

이번 기회에 이전 프로젝트 api 를 통해 여러 테스트를 진행해보며

실제 운영에서도 안정적으로 동작하는 개발을 하는 개발자에 가까워졌으면 좋겠다.
